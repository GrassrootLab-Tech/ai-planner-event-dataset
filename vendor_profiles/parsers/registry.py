from __future__ import annotations

from vendor_profiles.parsers.base import VendorProfileParser
from vendor_profiles.parsers.eventective import EventectiveProfileParser
from vendor_profiles.parsers.gigsalad import GigSaladProfileParser
from vendor_profiles.parsers.partyslate import PartySlateProfileParser
from vendor_profiles.parsers.thebash import TheBashProfileParser
from vendor_profiles.parsers.theknot import TheKnotProfileParser
from vendor_profiles.parsers.thumbtack import ThumbtackProfileParser
from vendor_profiles.parsers.weddingwire import WeddingWireProfileParser
from vendor_profiles.parsers.zola import ZolaProfileParser
from vendor_profiles.source_rules import normalize_source_host

PARSERS: dict[str, VendorProfileParser] = {
    p.source_host: p
    for p in (
        TheBashProfileParser(),
        EventectiveProfileParser(),
        GigSaladProfileParser(),
        PartySlateProfileParser(),
        TheKnotProfileParser(),
        ThumbtackProfileParser(),
        ZolaProfileParser(),
        WeddingWireProfileParser(),
    )
}


def get_parser_for_url(page_url: str) -> VendorProfileParser | None:
    """Return the hand-written parser for this URL's host, or None."""
    host = normalize_source_host(page_url)
    if not host:
        return None
    return PARSERS.get(host)
